...
function App() {
  ...
  useEffect(()=>{
    ...
    axios.get(`https://api.pokemontcg.io/v2/cards?q=name:${pokemon}&orderBy=-tcgplayer.prices.holofoil.market&pageSize=10`, {
      ...
      setLoading(false)
      console.log(pokemonCardsHolofoil)
      ...
      <div className='mt-12 md:flex mx-12' >
        ...
        <div className='md:w-2/5 md:ml-16' >
          <form onSubmit={submitSearch} className='flex h-10 '>
            <input className='flex-auto min-w-0 block w-full px-3 py-1.5  border border-black rounded transition ease-in-out m-0  focus:bg-white focus:border-black focus:outline-none' type="text" placeholder="Pokemon name"  value={searchText} onChange={e => setSearchText(e.target.value)}></input>
